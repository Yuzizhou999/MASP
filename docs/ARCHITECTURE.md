...
参考论文使用 `w=20`、`h=5` 个离散时间步。MASP 使用连续毫秒，需要通过车辆速度和地图尺度重新标定，而不是直接套用 20 秒和 5 秒。当前真实地图配置使用 `planningHorizonMs=120000`、`executionHorizonMs=5000`：前者覆盖 Jack 车辆穿越共享走廊并撤离到安全节点的完整计划，后者仍限制每轮对真实执行的承诺范围。
...
- 名义 `executionHorizonMs` 会延伸到下一个允许等待节点并记录为 `safeUntilMs`。当前 LM/AP 禁止等待，因此部分车辆需要一次承诺到 PP/CP。在线入口只提交截至该边界的完整动作段；边界后的候选尾部不进入正式预留表。
...
当前实施状态（2026-08-07）：已完成进程内在线调度 MVP，尚未达到阶段退出条件。

- `RollingHorizonPlanner.plan_cycle()` 将原批处理规划拆出为无副作用的单周期提案接口；它接收当前已释放任务、空闲车辆以及到达承诺边界的活动车辆，并在正式预留表副本上运行分配、优先级、SIPP 和安全节点提交逻辑。活动任务保持原车辆绑定。
- `OnlineDispatchRuntime` 按仿真时钟接收任务、每 `planningPeriodMs` 或承诺边界触发规划。完整 SIPP 计划只用于候选评分；发布和 ACK 的计划仅含截至 `safeUntilMs` 的完整动作段，正式预留仅安装这段前缀和终点安全保持。重复任务、重复 ACK 和陈旧车辆版本均有稳定错误码。
- `DeterministicSimulator.submit_task()`、`add_plan()` 和 `run_until()` 支持运行中注入任务与已确认计划。车辆执行到前缀边界后保留活动任务、载货状态和已完成的服务阶段；下一片段从当前安全节点继续规划。已取货任务只规划到放货和撤离，不会重复取货。空闲车辆遥测可用更高 revision 修正当前位置；道路中间仍不允许改路。
...
当前边界：已经实现“已承诺前缀不可变、未承诺尾部可重规划”的 RH-PP 语义，但重规划点必须是允许等待的安全节点，不能在道路中间改变正在执行的动作段。执行中遥测偏差、HTTP/gRPC 接口、控制器 ACK 超时与重连仍未实现，因此不能视为现场实时控制器。
...
- checkpoint v2 显式校验观察版本、`local_priority_prefix` 动作模式、奖励版本、规划窗口和 Top-M 长度，阶段 5 的完整排列模型不能再被静默用于阶段 7 前缀推理。
- 训练入口对每个小型冲突分量枚举 Top-M 前缀，用真实 SIPP/预留/验证结果生成 oracle 标签；完全可行状态提高采样权重，部分可行状态按已规划任务数和搜索成本保留学习信号。训练与验证状态独立拆分，默认使用约 3 万参数的小型图注意力模型，PPO 仅作为可选微调。
- RL 部署采用等预算混合候选：保留 `task_age`、`shortest_remaining`、`congestion` 和 `previous_order` 等确定性候选，用 RL 替代历史对照中未被选中的 random 候选。checkpoint 或推理失败时回退整套确定性候选，而非只回退单一 congestion。
- 三个配对 seed 的初始结果相对 `top_k`：平均任务周期下降 `3.30%`，平均排队下降 `15.39%`，SIPP 尝试下降 `6.02%`，资源冲突和规划超时均为 0；但按最后完成时刻计算的有效完成率下降 `6.23%`，因此 RL 吞吐退出条件仍未满足。
...
- 完整自动测试为 `112 passed`；RL 烟雾测试覆盖局部样本采集、行为克隆、PPO、Top-2 前缀 checkpoint、回载推理和零冲突回放。
